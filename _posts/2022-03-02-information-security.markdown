...
德诚VPN系统用户守则VPN系统建设和使用的根本目的，是方便公司的员工在校外网络环境下进行商业活动，并提高工作效率。为了更好地利用本系统，更充分地利用公司网络资源，各位VPN系统用户必须遵守如下几方面的条款：

第一条 本系统适用的用户范围，是公司所有员工。

第二条 本系统的直接管理和维护机构是德诚总部的IT运维组。

第三条 我司所有分公司已默认开通VPN，利用各自的用户名和密码，登录即可使用，详细操作方法请见《VPN使用方法》。

第四条 所有VPN用户在使用本系统过程中，必须遵守国家有关法律法规，遵守各级各部门相关规章制度；用户需对自己的VPN账户负责，并承担由此账户产生的网络行为及言论的相关责任。

第五条 用户必须保障本人VPN账户的密码安全，不得将本人申请的VPN帐号提供给他人使用；当发现自己的账户被盗用或有其他安全问题时，应立即通知VPN系统的管理机构进行处理。
...
3、远程访问SAP；
4、网络及应用系统的管理员在校外进行远程维护。 

除此之外的其他应用，需单独向信息部申请。

第七条 在通过VPN系统访问我司的电子资源过程中，不得滥用下载功能。如使用网络下载工具连续、集中、批量地进行下载电子资源，可能被数据库商认定为非法超量下载，并限制我司的资源访问。由此产生的损失和后果，由引发超量下载的具体VPN用户负责。 
...
第九条 用户提出申请、获得帐号即视为同意前列所有条款。

第十条 本守则由德诚总部IT运维组负责解释。
